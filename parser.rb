  ...
  class Parser
    ...
    def _propertyTypes
      ...
      return 1.upto(n).collect{|i|
        ...
        if MapiPropertyName[prop[:id]].nil?
          puts "Hmmm... unexpected property #{prop[:id]}"
          MapiPropertyName[prop[:id]] = "Unexepected#{prop[:id]}"
        end
    ...
    def _property(prop)
      ...
      return {type: prop[:type], id: prop[:id], name: prop[:name], value: value}
    end
